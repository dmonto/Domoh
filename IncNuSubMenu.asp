<nav class=submenu>
    <a title='<%=MesgS("Página Inicial","Go to home page")%>' href=TrLogOn.asp><%=MesgS("Inicio", "Home")%></a>|
	<a title='<%=MesgS("Finaliza la Sesión","Ends Session")%>' href=NuLogOff.asp><%=MesgS("Salir", "Logoff")%></a>	|
	<a title='<%=MesgS("Cómo conseguir ayuda","Shows Help Page")%>' href=NuAyuda.asp><%=MesgS("Ayuda", "Help")%></a>|
