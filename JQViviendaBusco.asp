...
<%		if UCase(rst("IdiomaEs"))="ON" then %>
			<img src=images/Espanol.gif alt='<%=MesgS("Anuncio en Español","Advert in Spanish")%>' class=icono />
<%	        
		end if 
		if UCase(rst("IdiomaEn"))="ON" then 
%>
			<img src=images/Ingles.gif alt='<%=MesgS("Anuncio en Inglés","Advert in English")%>' class=icono />
<%	    end if %>	
			</div>
		<div class=contmanuncioflexkid>
<%
