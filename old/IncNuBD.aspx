    ...
    sProviderOld="PROVIDER=SQLOLEDB;DATA SOURCE=dbsql5.hrn9.com;UID=domoh;PWD=hecaran;DATABASE=domoh"
...
Sub SelectProvincia(vTabla, vProvincia)
    <select title='@MesgS("Provincias disponibles","Available Regions")' name=provincia> 
        <option value=0 
            if vprovincia=0 then Response.Write "selected"
	...
	while not rst.Eof
		<option value=@rst("id") 
            if vprovincia=rst("id") then Response.Write " selected"
            >@rst("nombre")</option>
		rst.Movenext
	wend
	rst.Close
...
Sub SelectPais(vIdioma, vTabla, vPais)
    ...
    <select title='@MesgS("Países disponibles","Available Countries")' name=pais onchange='ClickPais();'> 
		<option value=0 
            if vPais=0 then Response.Write "selected"
            >
			@MesgS("-Cualquiera-","-Anyone-")</option>
	...
	while not rst.Eof
		<option value='@rst("id")'
            if vpais=rst("id") then Response.Write " selected"
            >@rst("nombre" & vIdioma)</option>
		rst.Movenext
	wend
	rst.Close
		</select>
...
Sub SelectTabla(vTabla, vPadre, vValor, vCampo)
    <select title='@vTabla @MesgS("disponibles","available")' name=@vTabla> 
		<option value=0 
            if vValor=0 then Response.Write "selected"
            >
			@MesgS("-Cualquiera-","-Anyone-")
	if vPadre<>"" then
	...
	while not rst.Eof
		<option value="@rst("id")" 
		    if vValor=rst("id") then Response.Write " selected"
            >@rst(vCampo)
		rst.Movenext
	wend
	rst.Close
...
sub SelectFuente(vValor)
	...
	while not rst.eof
		if rst("desc" & Session("Idioma")) <> "" then
			<option value="@rst("id")" 
                if vValor=rst("id") then Response.Write " selected"
                >@rst("desc" & Session("Idioma"))</option>
		end if
		rst.Movenext
	wend
	rst.Close
...
%>
	...
	<div>
		*@MesgS("Nombre de Contacto","Contact Name") 
        <input title='@MesgS("Tu nombre y apellidos","First and Last Names")' maxlength=35 name=nombre size=20 /> <input type=checkbox name=esagencia />@MesgS("Somos una Agencia","We are an Agency")</div>
	<div>@MesgS("Forma de Contacto","Contact Details")</div>
	<div>
        @MesgS("Teléfono 1","Phone #1"):  <input title='@MesgS("Teléfono principal","Main Phone Number")' maxlength=30 name=tel1 size=20 />
	...
	<div>*   e-Mail:  <input maxlength=60 name=email size=20 /><input type=checkbox name=mostraremail checked />@MesgS("Visible para Usuarios","Show in Adverts")</div>
	<div>
        <p>
    ...
    <div>* @MesgS("Nombre de Usuario","User Name"):  <input title='@MesgS("Nombre que te identificará","Id to identify yourself in domoh")' maxlength=60 name=usuario size=20 /></div>
    <div>* @MesgS("Elige una Clave","Choose a Password"):       <input type=password maxlength=20 size=10 name=password /></div>
    <div><div>* @MesgS("Tecléala de Nuevo","Type it again"):  <input type=password maxlength=20 size=10 name=password2 /></div><div>* @MesgS("¿Cómo nos conociste?","How did you get to Domoh?") @SelectFuente 0</div></div>
end sub
...
%>
	...
	elseif right(rst("banner"),3) = "oldswf" then 
		<object classid='clsid:D27CDB6E-AE6D-11cf-96B8-444553540000' codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,0,0">
			...
			<embed src='@rst("banner")' id=banner type=application/x-shockwave-flash pluginspace='http://www.macromedia.com/go/getflashplayer'></object>
	else 
