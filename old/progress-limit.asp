...
<%
'Sample file LimitProbar.ASP
'Session must be off to work correctly.
'IE-style progress bar for upload script.
' and file size limit. 
'This progress bar contains limit for each file.
'Files with size greater than MaxFileSize
'will be shown in red

	Const RefreshTime = 1'Seconds

  if Request.QueryString("UploadID") = "" then 
    response.redirect "Upload-Limit-File.ASP"
  end if
...
<td Align=Right VAlign=Top><A Href=http://www.motobit.com>Huge ASP file upload</A></td>
</tr></Table>
...
<%
else 'if Err = 0 then upload completted
%>
<HTML>
 <HEAD>
