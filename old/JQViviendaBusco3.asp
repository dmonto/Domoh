...
<body>
<div data-role=page>
	<div data-role=header><h1>Domoh - Búsqueda de Viviendas</h1></div><!-- /header -->
	<div role=main class=ui-content>
        <div class=row>
       	    <%=MesgS("Mira de forma gratuita","Look for free")%><% Response.Write MesgS(" las casas, pisos o habitaciones en casa compartidas que están disponibles", " houses, flats and rooms currently available")%>.</div>
        <form>
        <div class=row><%=MesgS("Pa&iacute;s","Country")%>:<% SelectPais Session("Idioma"), "Vivir", Session("Pais") %></div>
        <div class=row>
...
%>
        ...
        <div class=row>
	        <%=MesgS("M&aacute;ximo alquiler","Maximum rent")%> <% Response.Write MesgS("que estás dispuesto a pagar", "you are willing to pay")%>
		    <input title='<% Response.Write MesgS("Pon el precio en Euros","Rent in Euros")%>' size=5 name=precio />&euro;/<%=MesgS("Mes","Month")%></div></form>
<%
...
%>
        <div class=row>
	        <div class=row><input name=Submit type=submit class=btnLogin value='<%=MesgS("Buscar","Search")%>'/></div>
            Accordion
            <div id=accordion>
                <h3>Section 1</h3>
                <div>
                    <p>
                        Mauris mauris ante, blandit et, ultrices a, suscipit eget, quam. Integer ut neque. Vivamus nisi metus, molestie vel, gravida in, condimentum sit
                        amet, nunc. Nam a nibh. Donec suscipit eros. Nam mi. Proin viverra leo ut odio. Curabitur malesuada. Vestibulum a velit eu ante scelerisque vulputate.</p></div>
                <h3>Section 2</h3>
                <div>
                    <p>
                        Sed non urna. Donec et ante. Phasellus eu ligula. Vestibulum sit amet purus. Vivamus hendrerit, dolor at aliquet laoreet, mauris turpis porttitor velit, faucibus 
                        interdum tellus libero ac justo. Vivamus non quam. In suscipit faucibus urna.</p></div>
                <h3>Section 3</h3>
                <div>
                    <p>
                ...
                <div>
                    <p>
                        Cras dictum. Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Vestibulum ante ipsum primis in faucibus orci luctus et ultrices 
                        posuere cubilia Curae; Aenean lacinia mauris vel est.</p>
                    <p>Suspendisse eu nisl. Nullam ut libero. Integer dignissim consequat lectus. Class aptent taciti sociosqu ad litora torquent per conubia nostra, per inceptos himenaeos.</p></div></div>
            <div class=container>
...
%>
		        <a href=TrViviendaBuscoFront.asp class=linkutils><%=MesgS("B&uacute;squeda","Search")%> &gt; </a>
...
<%	        end if %>    
                    ...
                    <div>
        		        <div class=row>
                            <div class=left>
		    			        <a href=# onclick="window.open('QuAnuncioDetalle.asp?tabla=<%=sTabla%>&id=<%=rst("aId")%>','anunciospop','scrollbars=yes,width=700,height=550')"><img alt='Detalle' src=images/TrDetalle.gif /></a>
                                </div>
				    	    <div class=middle>
<%	        
...
<%	        if Request("tipo")<>"inquilino" then %>
                        <div class='middle <%=sClass%>' <%=sBack%>>
                            <a href="javascript:{sw1=window.open('TrMapa.asp?id=<%=rst("aId")%>','searchpage',
                                'toolbar=no,location=no,directories=no,status=no,scrollbars=yes,resizable=no,copyhistory=no,width=650,height=450'); sw1.focus();}">
...
<%
 ...
 %>                            
	    ...
	    <div data-role=footer><h4>Page Footer</h4></div><!-- /footer --></div></div><!-- /page -->
</body>
</html>
